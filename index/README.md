...
## Status: Phase 6 (cold start) complete

Phase 6 fixes the cold start: brand-new users land on a Welcome flow (real language
list with BCP-47 codes + auto direction, then a guided first-5-words form), no
sample data ships, a floating quick-add sheet works everywhere with context
category + collapsed "More", and bulk add accepts `word | native | meaning |
example`, CSV files, and Anki/Quizlet TSV with duplicate detection. Phase 5
(Review dashboard) and Phases 1–4 remain intact.
...
js/store.js                local cache, CRUD, counts, export/import, remote adapter, dup check
...
js/views/*.js             home, welcome (onboarding + first 5), category, learn
                          (hub/session/summary), review, manage, notebook
...
private cloud vocabulary sync, language setup (name + BCP-47 code + direction),
quick-add sheet, JSON export, JSON import (merge or replace), CSV + Anki/Quizlet
TSV bulk import with duplicate detection, erase-all with confirmation, multi-tab
sync, and a warning if the browser blocks local storage.

### Routes

| URI | Description |
|---|---|
| `#/welcome` | First-run onboarding — language picker, then guided first 5 words |
| `#/home` | Category grid (default for returning users; first run redirects to `#/welcome`) |
...
- Add one word with meaning, category, optional example, and per-word direction.
  Single adds warn on duplicates ("already have X — add anyway?").
- Quick-add FAB everywhere: word → meaning only, category pre-filled from context,
  native/example/direction behind "More", duplicates caught inline, stays open for
  rapid entry.
- Bulk: paste `term - meaning`, `word | native | meaning | example`, or tab-separated
  (Anki/Quizlet paste works); import CSV files (header-aware, quoted commas OK) and
  Anki/Quizlet TSV files; preview flags errors and skips duplicates (existing +
  in-batch) before confirming.
...
- Cold start is empty on purpose — no sample data. A brand-new user goes to
  `#/welcome` (language + first 5 words) instead of deleting someone else's Arabic.
