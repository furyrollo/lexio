  ...
  Activities.register({
    ...
    create: function (stage, ctx) {
          ...
          '<div class="quiz-options" role="group" aria-label="Answers">' +
            q.options.map(function (opt, n) {
              return '<button type="button" class="quiz-opt" data-n="' + n + '">' +
                '<span class="quiz-opt__key" aria-hidden="true"><span class="quiz-opt__num">' + (n + 1) + '</span>' +
                '<span class="quiz-opt__check">' + Icon('check') + '</span></span>' +
      ...
      function choose(n) {
        ...
        var wordLabel = WordDisplay.primary(q.word);
        note.textContent = correct
          ? 'Nice — that\u2019s ' + wordLabel
          : 'Not quite — the green answer is right.';
        note.classList.remove('is-good', 'is-bad');
        note.classList.add(correct ? 'is-good' : 'is-bad');
        if (correct) { score++; }
        showScore();
