...
(function (global) {
  ...
  /**
   * Duplicate detection for quick-add / single / bulk flows.
   * A word duplicates an existing one in the same language when its
   * transliteration OR its native script matches either spelling of an
   * existing entry (case-insensitive). Meaning is ignored: the same
   * spelling with a different gloss is still the same card.
   */
  function findDuplicate(data, languageId, excludeId) {
    var term = normKey(data && data.term);
    var native = normKey(data && (data.nativeScript != null ? data.nativeScript : data.native_script));
    if (!term && !native) { return null; }
    var langId = languageId || (state.settings && state.settings.activeLanguageId) || '';
    var found = null;
    state.words.some(function (w) {
      if (langId && w.languageId !== langId) { return false; }
      if (excludeId && w.id === excludeId) { return false; }
      var wTerm = normKey(w.term);
      var wNative = normKey(w.nativeScript);
      if (term && (term === wTerm || (wNative && term === wNative))) { found = w; return true; }
      if (native && (native === wTerm || (wNative && native === wNative))) { found = w; return true; }
      return false;
    });
    return found;
  }
