...
(function (global) {
  ...
  /* ---- Toast: pill with leading status dot -------------------------------- */
  function toast(message, opts) {
    opts = opts || {};
    var root = document.getElementById('toast-root');
    var el = document.createElement('div');
    el.className = 'toast';
    var kind = opts.icon === 'warning' || opts.icon === 'trash' || opts.icon === 'x_circle' ? 'warn'
      : (opts.icon === 'check' || opts.icon === 'check_circle' || !opts.icon ? 'ok' : 'info');
    var iconName = opts.icon || 'check';
    el.innerHTML = '<span class="toast__icon toast__icon--' + kind + '">' + Icon(iconName) + '</span>' +
