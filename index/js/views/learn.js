...
(function (global) {
  ...
  function modeHue(id) { return MODE_HUES[id] != null ? MODE_HUES[id] : 258; }
  ...
  function wordRow(result, index) {
    ...
    var boxLabel = '';
    try {
      var w = Store.getWord(result.id);
      if (w && global.Scheduler) { boxLabel = Scheduler.describeBox(Scheduler.boxOf(w)); }
    } catch (e) {}
    return '' +
      '<li class="sum-word">' +
        ...
        '</span>' +
        (boxLabel ? '<span class="tag">' + esc(boxLabel) + '</span>' : '') +
        '<span class="tag' + (result.correct ? ' tag--ok' : ' tag--edit') + '">' +
  ...
  function hubHTML(cat, pool) {
    ...
    var pills = '<div class="hero__stats learn-pills">' +
      '<span class="stat-pill"><span class="stat-pill__well stat-pill__well--words">' + Icon('cards') + '</span>' +
        '<b data-count="' + pool.length + '">0</b><span>in play</span></span>' +
      ...
      (due
        ? '<span class="stat-pill stat-pill--accent"><span class="stat-pill__well stat-pill__well--words">' + Icon('sparkle') + '</span>' +
          '<b data-count="' + due + '">0</b><span>due now</span></span>'
        : '') +
      '</div>';

    var chips =
      '<div class="scope-chips" role="tablist" aria-label="Practice scope">' +
        scopeChip(null, 'All words', Store.words().length, !cat) +
    ...
    var how = '' +
      '<details class="learn-how">' +
        ...
        '<p>Each word lives in one of six boxes. A right answer moves it up a box and pushes ' +
        ...
        '<ul class="learn-how__boxes">' +
          Scheduler.BOXES.map(function (b, i) {
            return '<li><b>' + (i + 1) + '</b><span>' + esc(b.label) + '<br>' +
              (b.days === 0 ? 'due now' : '+' + b.days + 'd') +
              '</span></li>';
          }).join('') +
        '</ul>' +
      '</details>';
  ...
  function renderPlay(root, params) {
    ...
    function leave() {
      /* Always return to the Learn hub — even for category-scoped sessions
         launched from Learn (via a scope chip). Going to /category/:id here
         dropped the user out of the Learn section entirely. Scope is kept in
         Store.settings.learnScopeId, so the hub reopens with the same filter.
         Sessions launched from a category page ("Practise these words") also
         land back in Learn, per request: back stays inside Learn. */
      if (cat && Store.settings.learnScopeId !== cat.id) {
        Store.setSetting('learnScopeId', cat.id);
      }
      Router.go('/learn');
    }
  ...
  function showSummary(root, o) {
    ...
    var gold = pct >= 90 && t.total > 0;
    var html = '' +
      '<section class="summary" aria-label="Session summary">' +
        '<div class="summary__hero">' +
          '<span class="summary__badge' + (gold ? ' summary__badge--gold' : '') + '" aria-hidden="true">' +
        ...
        '<div class="summary__stats">' +
          ...
          sumStat(t.correct, 'got it', 'summary__stat--good') +
          sumStat(wrong, 'again soon', 'summary__stat--bad') +
        '</div>' +
        '<div class="summary__ring-wrap"><span class="bar summary__bar"><span class="bar__fill" data-w="' +
          pct + '" style="inline-size:0%"></span></span><span class="summary__pct">0%</span></div>' +
        (t.results.length
    ...
    root.innerHTML = html;
    animateCounts(root);
    // Animate pct label with % suffix
    var pctEl = UI.$('.summary__pct', root);
    if (pctEl) {
      var reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
      if (reduced) { pctEl.textContent = pct + '%'; }
      else {
        var s0 = null;
        (function step(ts) {
          if (!s0) { s0 = ts; }
    ...
    function sumStat(value, label, cls) {
      return '<div class="summary__stat ' + cls + '"><b>' + value + '</b><span>' + esc(label) + '</span></div>';
    }
  }
