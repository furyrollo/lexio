...
(function (global) {
  ...
  var MODE_HUES = { flashcards: 258, quiz: 14, 'type-it': 190, match: 38, memory: 320 };

  function tile(icon, value, label, opts) {
    opts = opts || {};
    var numeric = /^\d/.test(String(value));
    return '' +
      '<' + (opts.href ? 'a href="' + opts.href + '"' : 'div') + ' class="rev-tile' +
        (opts.accent ? ' rev-tile--accent' : '') + '">' +
        '<span class="rev-tile__well rev-tile__well--' + (opts.well || 'accuracy') + '" aria-hidden="true">' + Icon(icon) + '</span>' +
        '<span class="rev-tile__head"><span>' + esc(label) + '</span></span>' +
        '<b' + (numeric ? ' data-count="' + parseInt(value, 10) + '"' : '') + '>' + value + '</b>' +
        (opts.sub ? '<span class="rev-tile__sub">' + esc(opts.sub) + '</span>' : '') +
        (opts.accent ? '<span class="rev-tile__arrow" aria-hidden="true">' + Icon('chevron') + '</span>' : '') +
      '</' + (opts.href ? 'a' : 'div') + '>';
  }
  ...
  function masteryRow(cat, dark) {
    ...
    var acc;
    if (cat.accuracy === null) {
      acc = '<span class="mastery-acc mastery-acc--quiet">not practised yet</span>';
    } else if (cat.accuracy >= 70) {
      acc = '<span class="mastery-acc mastery-acc--high">' + cat.accuracy + '% recall</span>';
    } else if (cat.accuracy >= 40) {
      acc = '<span class="mastery-acc mastery-acc--mid">' + cat.accuracy + '% recall</span>';
    } else {
      acc = '<span class="mastery-acc mastery-acc--low">' + cat.accuracy + '% recall</span>';
    }
    ...
    return '' +
      '<li class="mastery-row" style="' + vars + '">' +
        ...
        '<span class="mastery-row__text">' +
          ...
          '</span>' +
          '<span class="bar mastery-row__bar"><span class="bar__fill" data-w="' + pct + '" style="inline-size:0%"></span></span>' +
        '</span>' +
        acc +
      '</li>';
  }
  ...
  function dayChart(days) {
    ...
    var cells = days.map(function (d, idx) {
      ...
      var isToday = date.toDateString() === todayKey;
      ...
      try {
        ...
        if (!initial || initial.length > 2) { initial = date.toLocaleDateString(undefined, { weekday: 'short' }).charAt(0); }
      } catch (e) { initial = ['S','M','T','W','T','F','S'][date.getDay()]; }
      return '<div class="' + cls + '" title="' + when +
        ' · ' + d.answers + ' answers · ' + d.sessions + ' ' +
        UI.plural(d.sessions, 'session') + '">' +
        '<span class="rev-day__track"><i data-h="' + h + '" style="block-size:0%"></i></span>' +
        '<span class="rev-day__label">' + esc(initial) + (isToday ? ' · today' : '') + '</span>' +
      '</div>';
    }).join('');
  ...
  function boxPillCls(box) {
    if (box >= 5) { return 'box-pill--mastered'; }
    if (box >= 3) { return 'box-pill--familiar'; }
    if (box >= 1) { return 'box-pill--seen'; }
    return 'box-pill--new';
  }
  ...
  function needsList(items, langName) {
    ...
    return '' +
        ...
        items.map(function (w) {
          ...
          return '' +
            '<li class="sum-word">' +
              ...
              '</span>' +
              '<span class="tag tag--danger">missed ' + w.misses + '×</span>' +
              '<span class="tag box-pill ' + boxPillCls(w.box) + '">' + esc(Scheduler.describeBox(w.box)) + '</span>' +
            '</li>';
        }).join('') +
      '</ul>' +
      ...
      '<div class="acct-actions"><a class="btn btn--primary" href="#/learn">' + Icon('sparkle') + 'Drill these</a></div>';
  }
  ...
  function historyList(sessions) {
    ...
    return '<ul class="history-list">' + sessions.map(function (s) {
      ...
      var hue = MODE_HUES[s.mode] != null ? MODE_HUES[s.mode] : 258;
      ...
      var scoreCls = pct >= 80 ? 'tag--ok' : (pct >= 50 ? 'tag--gold' : 'tag--danger');
      return '' +
        '<li class="hist-row">' +
          '<span class="hist-row__icon" style="--mode-h:' + hue + '" aria-hidden="true">' + Icon(mode.icon) + '</span>' +
          '<span class="hist-row__text">' +
            ...
            '<span class="bar hist-row__bar"><span class="bar__fill" data-w="' + pct + '" style="inline-size:0%"></span></span>' +
          '</span>' +
          '<span class="tag ' + scoreCls + '">' + s.correct + '/' + s.total + '</span>' +
        '</li>';
    }).join('') + '</ul>';
  }
  ...
  function render(root) {
    ...
    if (!t.words) {
      root.innerHTML = head + UI.emptyState({
        ...
        actions: '<a class="btn btn--primary" href="#/welcome">' + Icon('plus') + 'Add your first words</a>'
      });
      return;
    }

    var tiles = '<div class="rev-tiles">' +
      tile('check', t.learned, 'words learned', { sub: 'of ' + t.words + ' collected', well: 'learned' }) +
      tile('flame', t.streak, UI.plural(t.streak, 'day') + ' in a row', { sub: 'practice streak', well: 'streak' }) +
      (snap.allTime.accuracy === null
        ? tile('target', '—', 'all-time accuracy', { sub: 'answers a session first', well: 'accuracy' })
        : tile('target', snap.allTime.accuracy + '%', 'all-time accuracy',
            { sub: snap.allTime.answers + ' answers so far', well: 'accuracy' })) +
      (snap.dueNow
        ? tile('bell', snap.dueNow, 'ready for review', {
            accent: true, href: '#/learn', sub: 'tap to practise them', well: 'due'
          })
        : tile('cards', t.words, 'in your collection', { sub: 'across ' + t.categoriesStarted + ' categories', well: 'accuracy' })) +
    '</div>';
    ...
    // Count-ups + bars + day/box heights (respect reduced motion)
    ...
    function paintBars() {
      UI.$$('.bar__fill[data-w]', root).forEach(function (f) { f.style.inlineSize = (f.dataset.w || 0) + '%'; });
      UI.$$('[data-h]', root).forEach(function (el) {
        el.style.transition = 'block-size .6s cubic-bezier(.16,1,.3,1)';
        el.style.blockSize = (el.dataset.h || 0) + '%';
      });
      UI.$$('.rev-box__track i', root).forEach(function (el, i) {
        // box heights are inline already except via data? ensure transition from 0
      });
    }
    if (reduced) { paintBars(); }
    else { requestAnimationFrame(function () { requestAnimationFrame(paintBars); }); }
    // Box chart: animate from 0
    (function () {
      var fills = UI.$$('.rev-box__track i', root);
      var finals = fills.map(function (el) { return el.style.blockSize; });
      if (reduced) { return; }
      fills.forEach(function (el) { el.style.blockSize = '0%'; });
      requestAnimationFrame(function () {
        requestAnimationFrame(function () {
          fills.forEach(function (el, i) { el.style.blockSize = finals[i]; });
        });
      });
    })();
  }
